...
**Exception — tracked login/config via links:** [`config/remicsdev/mics/`](../../config/remicsdev/mics/) — `web.config` is a **real file in the repo** (IIS on `D:` symlinks to it); `Tlogin.aspx*` are repo symlinks to live files. See [`config/remicsdev/README.md`](../../config/remicsdev/README.md) and `scripts/New-RemicsDevConfigLinks.ps1`.
